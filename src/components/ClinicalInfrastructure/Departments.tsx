...
const getDeptIcon = (name: string) => {
  ...
  if (n.includes('cardio') || n.includes('heart') || n.includes('emergenc')) return <HeartIcon />;
  if (n.includes('neuro') || n.includes('brain') || n.includes('psych')) return <BrainIcon />;
  if (n.includes('radio') || n.includes('scan') || n.includes('lab')) return <RadioIcon />;
  return <HeartIcon />;
};
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-pulse">
        {Array(4).fill(0).map((_, i) => (
          <div key={i} className="bg-slate-50 h-[300px] rounded-[32px] border border-slate-100"></div>
        ))}
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="bg-rose-50 p-12 rounded-[40px] border border-rose-100 text-center">
         <h4 className="text-xl font-black text-rose-600 mb-2">Protocol De-synchronization</h4>
         <p className="text-rose-400 font-medium">Failed to retrieve department registry. Verify clinical network state.</p>
      </div>
    );
  }
  ...
  if (!departments || departments.length === 0) {
    return (
        <div className="bg-slate-50 p-12 rounded-[40px] border border-dashed border-slate-200 text-center">
           <h4 className="text-xl font-black text-slate-400">Registry Vacuum Detected</h4>
           <p className="text-slate-300 font-medium mt-2">Initialize departments to begin institutional scaling.</p>
        </div>
      );
  }
          ...
          <div key={dept.id} className="bg-white p-8 rounded-[32px] border border-slate-200/60 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              ...
              <div className="flex justify-between items-center text-[0.7rem] font-bold">
                 <span className="text-slate-400">{dept.staffCount} Active Personnel</span>
      ...
      <div className="bg-white p-10 rounded-[40px] border border-slate-200/60 shadow-sm">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="p-8 bg-slate-50 rounded-[32px] border border-slate-100">
               <h5 className="text-[0.65rem] font-black text-slate-400 uppercase tracking-widest mb-4">Institutional Topology</h5>
               <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center font-black">
                     {departments.length}
                  </div>
                  <span className="font-bold text-slate-800 text-lg underline decoration-emerald-200 underline-offset-4">Department Nodes</span>
               </div>
            </div>
            <div className="p-8 bg-slate-50 rounded-[32px] border border-slate-100">

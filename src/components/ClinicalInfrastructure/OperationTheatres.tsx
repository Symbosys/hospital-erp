...
const OT_DATA = [
  { id: "OT-01", status: "Active Surgery", doctor: "Dr. Elena Gilbert", patient: "PT-4105", procedure: "Neuro-Dissection", progress: 65, equipment: "Excellent" },
  { id: "OT-02", status: "Ready", doctor: "-", patient: "-", procedure: "-", progress: 0, equipment: "Ready" },
  { id: "OT-03", status: "Cleaning", doctor: "-", patient: "PT-4221", procedure: "Cardio-Stent (Completed)", progress: 100, equipment: "Observation" },
];
    ...
    <div className="bg-white rounded-[40px] border border-slate-200/60 shadow-sm overflow-hidden animate-fade">
      <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
        ...
        <div className="flex gap-4">
           <button className="bg-white border border-slate-200 px-6 py-3 rounded-2xl font-bold text-xs text-slate-700 shadow-sm">Surgery Logs</button>
           <button className="bg-primary text-white px-8 py-3 rounded-2xl font-black text-xs shadow-lg shadow-primary-glow">Book OT Slot</button>
        </div>
      </div>
        ...
        {OT_DATA.map((ot) => (
          <div key={ot.id} className="bg-white p-8 rounded-[36px] border border-slate-200/60 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
            ...
            <div className="flex justify-between items-start mb-8 pl-4">
               <div>
                  <h4 className="text-2xl font-black text-slate-900">{ot.id}</h4>
                  <p className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest mt-1">Surgical Node Capacity</p>
               </div>
               <div className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest ${
                 ot.status === 'Active Surgery' ? 'bg-rose-50 text-rose-600 border border-rose-100' :
                 ...
                 <>
                   <div className="flex justify-between items-center">
                      <div className="space-y-1">
                         ...
                         <p className="text-sm font-black text-slate-800 underline decoration-primary/20 underline-offset-4">{ot.doctor}</p>
                      </div>
                      <div className="text-right space-y-1">
                         <span className="text-[0.65rem] font-black text-slate-400 uppercase tracking-widest">Procedural Code</span>
                         <p className="text-sm font-black text-slate-800">{ot.procedure}</p>
                      </div>
                   </div>
                   ...
                   <div className="space-y-3">
                      ...
                      <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                         <div className="bg-rose-500 h-full transition-all duration-3000" style={{ width: `${ot.progress}%` }}></div>
                      </div>
                   </div>
                 </>
               ) : ot.status === "Ready" ? (
            ...
            <div className="mt-8 flex justify-between items-center pl-4 pr-4">
               ...
               <button className="text-primary font-black text-xs hover:underline transition-all">Telemetry View</button>
            </div>
          </div>
        ))}
      </div>
